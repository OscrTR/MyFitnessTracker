...
class PeriodStats extends Equatable {
  ...
  factory PeriodStats.fromTrainings(List<HistoryTraining> trainings) {
    ...
    final runTrainings =
        trainings.where((t) => t.trainingType == TrainingType.run).toList();
    final workoutTrainings =
        trainings.where((t) => t.trainingType == TrainingType.workout).toList();
    final yogaTrainings =
        trainings.where((t) => t.trainingType == TrainingType.yoga).toList();
    ...
    final yogaUniqueExercises = yogaTrainings.fold<Set<int>>(
      {},
      (exerciseSet, t) => exerciseSet..addAll([t.exercisesNumber]),
    ).length;
